...
`include "led_gpio.v"
module top #(
    // Program image loaded into progmem at simulation start. Overridable so a
    // testbench can run a different firmware without editing the RTL.
    parameter MEM_FILE = "firmware.hex"
)(
    // NOTE: every submodule is instantiated below as .rst(!rst), so despite
    // the historical comment this port is ACTIVE-LOW at the SoC boundary:
    // drive it to 0 to hold the system in reset, 1 to run.
    input rst, clk,           // Reset (active low at this port), system clock
  ...
  // Program Memory
  // -------------------------------
  progmem #(.MEM_FILE(MEM_FILE)) mem0(
    .rst(!rst), .clk(clk),
    .addr(addr),
    .data_in(mem_wdata),
