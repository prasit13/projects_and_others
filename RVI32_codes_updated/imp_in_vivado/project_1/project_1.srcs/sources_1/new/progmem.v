...
module progmem(
  ...
  // Image loaded at simulation start. Parameterized so a testbench can select
  // a different program without editing the RTL.
  ...
  initial begin
    // Read hex values from the selected image into the PROGMEM array.
    // Each line contains one 32-bit instruction/data word in hexadecimal.
    $readmemh(MEM_FILE, PROGMEM);
  end
