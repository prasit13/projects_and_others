...
module cpu(
  ...
  // RV32I mandates that the shift amount is the LOW FIVE BITS of the source,
  // for both the register (R-type) and immediate (I-type) forms. Using the
  // full 32-bit rs2 value for R-type shifts would return 0 whenever rs2 >= 32
  // instead of shifting by rs2 mod 32.
  wire [31:0] shift_amt = {27'b0, alu_in2[4:0]};
  ...
  //
  // ADDRESS GENERATION MUST BE TESTED FIRST. For loads, stores and JALR the
  // funct3 field encodes the ACCESS WIDTH (or is unused), not an ALU function,
  // so those encodings collide with the arithmetic cases below. Placing this
  // case last let the collisions win: SW (funct3=010) fell into the SLT case
  // and produced a comparison result instead of base+offset, so every word
  // store went to the wrong address; LH/LHU (001) fell into SLL and LBU (100)
  // into XOR. For all three instruction classes the ALU must simply add the
  // base register to the sign-extended offset.
  wire [31:0] alu_result = (isStype | isLtype | isJALR)               ? ADD :
                           (funct3==3'b000 && isRtype && ~funct7[5])  ? ADD :
                           (funct3==3'b000 && isItype)                ? ADD :
                           (funct3==3'b000 && funct7[5])              ? SUB[31:0] :
                           (funct3==3'b100) ? XOR :
                           (funct3==3'b110) ? OR :
                           (funct3==3'b111) ? AND :
                           (funct3==3'b010) ? {31'b0, LESS_THAN} :
                           (funct3==3'b011) ? {31'b0, LESS_THAN_U} :
                           (funct3==3'b001) ? SLL :
                           (funct3==3'b101 && ~funct7[5]) ? SRL :
                           (funct3==3'b101 && funct7[5]) ? SRA : 32'b0;
