...
module stageDecode (
    ...
    input logic [`BP_BUS_W-1:0] bpBusF,
    output logic [`BP_BUS_W-1:0] bpBus,
    // N4: the perceptron's late verdict for the instruction in this stage
    input logic bpOvrValid,
    input logic bpOvrTaken,
  ...
  // Fetch recorded the fast prediction. If the perceptron overrode it this
  // cycle, the instruction moving on to EX must carry the corrected value,
  // otherwise EX would compare the outcome against a prediction that was
  // never actually followed and report a misprediction that did not happen.
  assign bPredictedTaken = bpOvrValid ? bpOvrTaken : bPredictedTakenSv;
  assign bpBus = bpBusSv;
