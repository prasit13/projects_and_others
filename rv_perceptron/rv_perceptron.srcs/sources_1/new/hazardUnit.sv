module hazardUnit(  output logic [1:0] fwdAE,
  ...
  // An override resteers fetch, so whatever is in F this cycle is wrong-path
  // and must not enter D. That is the single-bubble cost of N4. The
  // instruction in D itself is not killed -- it carries on to EX with its
  // corrected prediction.
  assign flushD = bStall|ovrValidD;
