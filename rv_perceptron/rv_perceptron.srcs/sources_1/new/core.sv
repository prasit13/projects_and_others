...
module core(
  ...
  // speculative-history side bus, F -> D -> E
  logic [`BP_BUS_W-1:0] P_bpBus, D_bpBus, E_bpBus;
  // N4: perceptron override, produced while the instruction is in DECODE
  logic        P_ovrValid, P_ovrTaken;
  logic [31:0] P_ovrTarget;
  // with PREDICTOR=0 there is nothing to consult, so fetch falls back to the
  // resolve-in-EX redirect path regardless of the testbench request
  logic usePredictorInt;
  assign usePredictorInt = usePredictor && (`PREDICTOR != 0);
  ...
  stageDecode decode (
                ...
                .bpBusF             (P_bpBus),
                .bpBus              (D_bpBus),
                .bpOvrValid         (P_ovrValid),
                .bpOvrTaken         (P_ovrTaken),
                .rdAddrW            (W_rdAddr),
                .regWriteW          (W_regWrite),
                .rdW                (W_rd),
                .rdAddr             (D_rdAddr),
  ...
  //=====================================================================
  // Branch predictor. Select with -DPREDICTOR=<n> (see bp_config.vh):
  //   0 = static not-taken, 1 = bimodal, 2 = gshare, 3 = perceptron
  //=====================================================================
  generate
    if (`PREDICTOR == 0)
    begin : gNoPredict
      assign P_bPredictTaken = 1'b0;
      assign P_btbTarget     = 32'b0;
      assign P_bpBus         = '0;
      assign P_ovrValid      = 1'b0;
      assign P_ovrTaken      = 1'b0;
      assign P_ovrTarget     = 32'b0;
    end
    else if (`PREDICTOR == 1 || `PREDICTOR == 2)
    begin : gClassic
      classicPredictor #(.MODE(`PREDICTOR - 1)) bPredict (
                         .clk           (clk),
                         .rst           (rst),
                         .fetchPc       (F_pc),
                         .fetchStall    (H_stallF),
                         ...
                         .exBranch      (E_branch),
                         .exTaken       (E_btbUpdate),
                         .exWrong       (E_wrongBranch),
                         .exPc          (E_pc),
                         .exTarget      (E_btbTarget),
                         .exBpBus       (E_bpBus),
                         .exControlXfer (E_controlXfer)
                       );
    end
    else
    begin : gPerceptron
      perceptronPredictor bPredict (
                            .clk           (clk),
                            .rst           (rst),
                            .fetchPc       (F_pc),
                            .fetchStall    (H_stallF),
