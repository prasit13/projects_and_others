...
for i = 1:2
    ...
    xlabel('Frequency');
    ylabel('Rate');
    title(['Rate vs Frequency for ', num2str(BF(i)), 'Hz ANF']);
    hold off;
end
...
for i = 1:2
    for j = 1:10
        [~, idx] = min(abs(tones - BF(i)));  % Find exact closest frequency
        rate_int(j,i) = rate_matrix(idx, j, i);
    end
end
figure(3)
hold on;
...
% Separating out 'aaah', generating different intensity stimuli 
